...
;;; Time-stamp: "2002-07-03 12:53:43 maggesi"
...
;;
;; C-return (`coq-add-line')     send the current line and insert ret.
;; M-return (`coq-send-line')    send the current line.
;; C-down   (`coq-send-line')    send the current line.
;; C-up     (`coq-send-undo')    send the command "Undo".
;; C-right  (`coq-come-here')    restart and send until current point.
;; C-left   (`coq-send-abort')   send the command "Abort".
;; C-c C-r  (`coq-send-region')  send the current region.
;; C-c C-a  (`coq-send-abort')   send the command "Abort".
;; C-c C-t  (`coq-send-restart') send the command "Restart".
;; C-c C-s  (`coq-send-show')    send the command "Show".
;; C-c C-u  (`coq-send-undo')    send the command "Undo".
;; C-c C-v  (`coq-check-region') run command "Check" on region.
;; C-c C-v  (`coq-check-print')  run command "Print" on region.
;; C-c .    (`coq-come-here')    restart and send until current point.
...
(setq coq-byte-program-name "coqtop -byte")
...
(defvar inferior-coq-mode-map
  (let ((m (make-sparse-keymap)))
    ...
    (define-key m "\C-c\C-p" 'coq-print-region)
    (define-key m [(control up)] 'coq-send-undo)
    (define-key m [(control left)] 'coq-send-abort)
    m))

;; Install the process communication commands in the coq-mode keymap.
(define-key coq-mode-map [(control return)] 'coq-add-line)
(define-key coq-mode-map [(meta return)] 'coq-send-line)
(define-key coq-mode-map [(control down)] 'coq-send-line)
(define-key coq-mode-map [(control up)] 'coq-send-undo)
(define-key coq-mode-map [(control right)] 'coq-come-here)
(define-key coq-mode-map [(control left)] 'coq-send-abort)
(define-key coq-mode-map [f8] 'coq-send-vernac)
(define-key coq-mode-map [f7] 'coq-send-replay)
...
(define-key coq-mode-map "\C-c\C-p" 'coq-print-region)
(define-key coq-mode-map "\C-c." 'coq-come-here)

'(easy-menu-define coq-menu inferior-coq-mode-map "Menu for `inferior-coq'"
  '("Coq"
    ["One entry" coq-send-region t]
    '("Sub Menu"
      ["My subentry" my-obscure-function t])
    ))

(easy-menu-define inferior-coq-menu inferior-coq-mode-map
  "Menu for `inferior-coq-mode'."
  '("Inf-Coq"
    ["Abort" coq-send-abort t]
    ["Restart" coq-send-restart t]
    ["Show" coq-send-show t]
    ["Undo" coq-send-undo t]
    ["Check region" coq-check-region t]
    ["Print region" coq-print-region t]
    ))

(easy-menu-define coq-menu coq-mode-map
  "Menu for `coq-mode'."
  '("Inf-Coq"
    ["Run Coq" run-coq-other-frame t]
    ["Run Coq Byte" run-coq-byte-other-frame t]
    ["Send line" coq-send-line t]
    ["Send Region" coq-send-region t]
    ["Come here" coq-come-here t]
    ["Abort" coq-send-abort t]
    ["Restart" coq-send-restart t]
    ["Show" coq-send-show t]
    ["Undo" coq-send-undo t]
...
Functions and key bindings (Learn more keys with `C-c C-h'):
  ...
  C-c C-p  (`M-x coq-print-region') run command \"Print\" on region.
...
"
  ...
  ;; (easy-menu-add (inferior-coq-menu "inferior-coq-mode"))
  ;; (easy-menu-add (coq-menu "coq-mode"))
  )
...
(defun run-coq-byte-other-frame (cmd)
  "Run an inferior Coq process, input and output via buffer *coq*.
If there is a process already running in `*coq*', switch to that buffer.
With argument, allows you to edit the command line (default is value
of `coq-program-name').  Runs the hooks `inferior-coq-mode-hook'
...
\(Type \\[describe-mode] in the process buffer for a list of commands.)"
  ...
  (interactive (list (if current-prefix-arg
			 (read-string "Run Coq: " coq-byte-program-name)
			 coq-byte-program-name)))
  (if (not (comint-check-proc "*coq*"))
      (let ((cmdlist (coq-args-to-list cmd)))
	(set-buffer (apply 'make-comint "coq" (car cmdlist)
			   nil (cdr cmdlist)))
...
(defun coq-add-line ()
  "Send the current line to the Coq process then insert return."
  (interactive)
  (save-excursion
    (end-of-line)
    (let ((end (point)))
...
(defun coq-send-vernac ()
  "Send vernacular phrase at point."
  (interactive)
  (coq-backward-vernac)
  (while (forward-comment 1))
  (let ((beg (point))) 
    (coq-forward-vernac)
    (coq-send-region beg (point))
    (coq-forward-vernac)))
...
(defun coq-send-replay ()
  "Send the command \"Restart\" and move point at the beginning of the
current proof."
  (interactive)
  (coq-send-restart)
...
(defun coq-print-region (start end)
  "Run the commmand \"Check\" on the current region."
  (interactive "r")
  (comint-send-string (coq-proc) "Print ")
  (comint-send-region (coq-proc) start end)
  (comint-send-string (coq-proc) ".\n"))
